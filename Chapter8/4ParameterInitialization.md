...
> **[warning]** 没有输入模式丢失在前向传播的零空间中，以及没有梯度模式丢失在反向传播的零空间中?  
...
> **[success]** [Gram-Schmidt正交化](https://windmissing.github.io/mathematics_basic_for_ML/LinearAlgebra/orthogonal.html)  
先随机初始化一组权重，然后用Gram-Schmidt正交化方法把这组权重正交化。  

与之相比，在高维空间上使用高熵分布来随机初始化，计算代价更小并且不太可能分配单元计算彼此相同的函数。
> **[sucess]** [熵](https://windmissing.github.io/mathematics_basic_for_ML/Information/entropy.html)  
例如高斯分布就是一种高熵分布。  
...
而~{GlorotAISTATS2010-small}建议使用**标准初始化**（normalized initialization）
$$
\begin{aligned}
...
> **[success]**  
> Ag视频中说：  
> 如果使用sigmoid/tanh Unit, 方差为$\frac{1}{n}$  
> 如果使用ReLU，方差为$\frac{2}{n}$  
> 还有一种，没说给什么Unit用，方差为$\sqrt {\frac{2}{n^l + n^{l-1}}}$  
> 其中n为第l层的Unit数    
> 方差也可以作为一个超参数，但作为超参数调优效果一般。  
