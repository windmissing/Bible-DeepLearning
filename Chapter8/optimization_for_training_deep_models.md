...
因此，我们只是间接地优化$P$。
我们希望通过降低一个不同的代价函数 $J(\Vtheta)$来提高$P$。
...
\subsection{代理损失函数和提前终止}
\label{sec:surrogate_loss_functions_and_early_stopping}
% 269 end
有时，我们真正关心的损失函数（比如分类误差）并不能被高效地优化。
例如，即使对于线性分类器而言，精确地最小化$0-1$损失通常是不可解的（复杂度是输入维数的指数级别）\citep{Marcotte-92}。
...
回想一下，$n$个样本均值的标准差（\eqnref{eq:5.46}）是$\sigma/\sqrt{n}$，其中$\sigma$是样本值真实的标准差。
...
随机方法的典型示例是随机梯度下降，这将在\secref{sec:stochastic_gradient_descent_chap8}中详细描述。
% 272 mid
...
\begin{itemize}
    ...
    \item 极小批量通常难以充分利用多核架构。
    这促使我们使用一些绝对最小批量，低于这个值的小批量处理不会减少计算时间。
    
    \item 如果批量处理中的所有样本可以并行地处理（通常确是如此），那么内存消耗和批量大小会正比。
...
% 272 mid    
    \item 
    可能是由于小批量在学习过程中加入了噪声，它们会有一些正则化效果~\citep{Wilson-2003}。
    泛化误差通常在批量大小为$1$时最好。
...
它将遵循着真实\emph{泛化误差}（\eqnref{eq:8.2}）的梯度。
...
在这种情况下，泛化误差（\eqnref{eq:8.2}）可以表示为
\begin{equation}
\label{eq:8.7}
...
\end{equation}
以此获得泛化误差准确梯度的无偏估计。
最后，在方向$\hat{\Vg}$上更新$\Vtheta$也就意味着以（真实）泛化误差为目标进行\,SGD\,。
%  274 end
...
读者也可以参考~\cite{bottou-bousquet-2008}中关于训练样本数目增长时，泛化误差上计算瓶颈影响的讨论。
% -- 275 mid
...
这是数值优化、凸优化或其他形式的优化中普遍存在的问题，更多细节请回顾\secref{sec:beyond_the_gradient_jacobian_and_hessian_matrices}。
% -- 275 mid
...
回顾\eqnref{eq:4.9}，代价函数的二阶泰勒级数展开预测梯度下降中的$-\epsilon\Vg$步骤会增加
\begin{equation}
...
我们可以构建没有隐藏单元的小规模神经网络，其局部极小值的代价比全局最小点的代价大很多~\citep{Sontag-cs89,Brady89,Gori-pami91}。
...
大部分局部极小值都具有很小的代价函数，我们能不能找到真正的全局最小点并不重要，而是需要在参数空间中找到一个代价很小（但不是最小）的点~\citep{Saxe-et-al-ICLR13,Dauphin-et-al-NIPS2014-small,GoodfellowOptimization15,Choromanska-et-al-AISTATS2015}。
% 278 head
...
以上现象出现在许多种类的随机函数中。
那么是否在神经网络中也有发生呢？
\cite{Baldi89}从理论上证明，不具非线性的浅层自编码器（\chapref{chap:autoencoders}中将介绍的一种将输出训练为输入拷贝的前馈网络）只有全局极小值和鞍点，没有代价比全局极小值更大的局部极小值。
他们还发现这些结果能够扩展到不具非线性的更深的网络上，不过没有证明。
这类网络的输出是其输入的线性函数，但它们仍然有助于分析非线性神经网络模型，因为它们的损失函数是关于参数的非凸函数。
这类网络本质上是多个矩阵组合在一起。
\cite{Saxe-et-al-ICLR13}精确解析了这类网络中完整的学习动态，表明这些模型的学习能够捕捉到许多在训练具有非线性激活函数的深度模型时观察到的定性特征。
\cite{Dauphin-et-al-NIPS2014-small}通过实验表明，真实的神经网络也存在包含很多高代价鞍点的损失函数。
\cite{Choromanska-et-al-AISTATS2015}提供了额外的理论论点，表明另一类和神经网络相关的高维随机函数也满足这种情况。
...
\cite{GoodfellowOptimization15}可视化了最新神经网络的几个学习轨迹，\figref{fig:chap8_plot_atmu_relu_5}\,给了一个例子。
这些可视化显示，在突出的鞍点附近，代价函数都是平坦的，权重都为零。
但是他们也展示了梯度下降轨迹能够迅速逸出该区间。
\cite{GoodfellowOptimization15}也主张，应该可以通过分析来表明连续时间的梯度下降会逃离而不是吸引到鞍点，但对梯度下降更现实的使用场景来说，情况或许会有所不同。
...
\cite{Dauphin-et-al-NIPS2014-small}介绍了二阶优化的无鞍牛顿法，并表明和传统版本的牛顿法相比有显著改进。
...
% 281 head
多层神经网络经常存在像悬崖一样的斜率较大区域，如\figref{fig:chap8_cliff}所示。
这是由于几个较大的权重相乘导致的。
...
不管我们是从上还是从下接近悬崖，情况都很糟糕，但幸运的是我们可以用使用\secref{sec:clipping_gradients}介绍的启发式梯度截断来避免其严重的后果。
...
深层的计算图不仅存在于前馈网络，还存在于之后介绍的循环网络中（在\chapref{chap:sequence_modeling_recurrent_and_recursive_nets}中描述）。
...
所以即使使用非常深层的前馈网络，也能很大程度上有效地避免梯度消失与爆炸问题~\citep{Sussillo14}。

% 282 mid

在更详细地描述循环网络之后，我们将会在\secref{sec:the_challenge_of_long_term_dependencies}进一步讨论循环网络训练中的挑战。
...
对于具有离散的$y$和~\ENNAME{softmax}~分布$p(y\mid\Vx)$的分类器而言，若模型能够正确分类训练集上的每个样本，则负对数似然可以无限趋近但不会等于零。
同样地，实值模型$p(y\mid\Vx) = \mathcal{N}(y;f(\Vtheta),\beta^{-1})$的负对数似然会趋向于负无穷——如果$f(\Vtheta)$能够正确预测所有训练集中的目标$y$，学习算法会无限制地增加$\beta$。
\figref{fig:chap8_bad_global}给出了一个失败的例子，即使没有局部极小值和鞍点，该例还是不能从局部优化中找到一个良好的代价函数值。
...
之前我们已经介绍了梯度下降（\secref{sec:gradient_based_optimization}），即沿着整个训练集的梯度方向下降。
这可以使用随机梯度下降很大程度地加速，沿着随机挑选的小批量数据的梯度下降方向，就像\secref{sec:stochastic_gradient_descent_chap5}和\secref{sec:batch_and_minibatch_algorithms}中讨论的一样。
...
如\secref{sec:batch_and_minibatch_algorithms}中所讨论的，按照数据生成分布抽取$m$个小批量（独立同分布的）样本，通过计算它们梯度均值，我们可以得到梯度的无偏估计。
...
\cite{bottou-bousquet-2008-small}因此认为对于机器学习任务，不值得探寻收敛快于$O(\frac{1}{k})$的优化算法——更快的收敛可能对应着过拟合。
...
了解\,SGD\,更多的信息，请查看~\cite{Bottou98}。
...
动量方法~\citep{polyak1964some}旨在加速学习，特别是处理高曲率、小但一致的梯度，或是带噪声的梯度。
...
比较\figref{fig:chap4_poor_conditioning_color}，它也显示了没有动量的梯度下降的行为。
}
...
带动量的\,SGD\,算法如\algref{alg:momentum}所示。
...
完整的\,Nesterov 动量算法如\algref{alg:nesterov}所示。
% 292 mid
...
在凸批量梯度的情况下，Nesterov 动量将额外误差收敛率从$O(1/k)$（$k$步后）改进到$O(1/k^2)$，如~\cite{Nesterov83b}所示。
...
回顾\secref{sec:early_stopping}，在某些模型上，提前终止的梯度下降等价于权重衰减。
...
而~\citet{GlorotAISTATS2010-small}建议使用标准初始化, % ?
\begin{equation}
...
如果该随机游走调整到保持范数，那么前馈网络能够很大程度地避免相同权重矩阵用于每层的梯度消失与爆炸问题，如\secref{sec:long_term_dependencies}所述。
% 295 end
...
\cite{martens2010hessian-small}提出了一种被称为稀疏初始化的替代方案，每个单元初始化为恰好有$k$个非零权重。
...
\begin{itemize}
\item 如果偏置是作为输出单元，那么初始化偏置以获取正确的输出边缘统计通常是有利的。
...
\item 有时，我们可能想要选择偏置以避免初始化引起太大饱和。
...
否则，$u$没有机会学习。
例如，\cite{Jozefowicz-et-al-ICML2015}提议设置~LSTM~模型遗忘门的偏置为$1$，如\secref{sec:the_long_short_term_memory_and_other_gated_rnns}所述。
\end{itemize}
% 297 end
...
\textbf{Delta-bar-delta}算法\citep{jacobs1988}是一个早期的在训练时适应模型参数各自学习率的启发式方法。
...
\subsection{AdaGrad}
\label{sec:adagrad}
\textbf{AdaGrad}算法，如\algref{alg:ada_grad}所示，独立地适应所有模型参数的学习率，按照每个参数的梯度历史值的平方和的平方根成反比缩放每个参数\citep{Duchi+al-2011}。
...
\subsection{RMSProp}
\label{sec:rmsprop}
\textbf{RMSProp}算法\citep{Hinton-ipam2012}修改AdaGrad以在非凸设定下效果更好，改变梯度积累为指数加权的移动平均。
...
\end{equation}
最速下降法中$\MH$是$J$相对于$\Vtheta$的\,Hessian\,矩阵在$\Vtheta_0$处的估计。
...
如果目标函数是凸的但非二次的（有高阶项），该更新将是迭代的，得到和牛顿法相关的算法，如\algref{alg:newton}所示。
...
在\secref{sec:plateaus_saddle_points_and_other_flat_regions}，我们讨论了牛顿法只适用于\,Hessian\,矩阵是正定的情况。
...
\end{equation}
这个正则化策略用于牛顿法的近似，例如Levenberg-Marquardt算法\citep{Levenberg44,Marquardt63}，只要\,Hessian\,矩阵的负特征值仍然相对接近零，效果就会很好。
...
这解决了\figref{fig:chap4_poor_conditioning_color}中使用固定学习率所遇到的一些问题，但即使使用最佳步长，算法仍然朝最优方向曲折前进。
...
另外，尽管（非线性）共轭梯度法传统上作为批方法，小批量版本已经成功用于训练神经网络~\citep{Le-ICML2011}。
针对神经网路的共轭梯度法应用早已被提出，例如缩放的共轭梯度法\citep{Moller}。
...
BFGS\,近似的说明和推导出现在很多关于优化的教科书中，包括\cite{Lue84}。

当\,Hessian\,逆的近似$\MM_t$更新时，下降方向$\Vrho_t$为$\Vrho_t = \MM_t \Vg_t$。
...
L-BFGS\,算法使用和\,BFGS\,算法相同的方法计算$\MM$的近似，但开始时假设上一步的$\MM$即$\MM^{(t-1)}$是单位矩阵，而不是每一步都为下一步存储近似。
...
批标准化~\citep{Ioffe+Szegedy-2015}是优化深度神经网络中最激动人心的最新创新之一。
...
事实上，这是~\cite{Desjardins2015}中采用的方法，为批标准化提供了灵感。
...
在贪心监督预训练的原始版本\citep{Bengio-nips-2006-short}中，每个阶段包括一个仅涉及最终神经网络的子集层的监督学习训练任务。
贪心监督预训练的一个例子如\figref{fig:chap8_deep_sup}所示，其中每个附加的隐藏层作为浅层监督多层感知机的一部分预训练，以先前训练的隐藏层输出作为输入。
\cite{Simonyan2015}~预训练深度卷积网络（11层权重），然后使用该网络前四层和最后三层初始化更深的网络（多达19层权重），并非一次预训练一层。
非常深的新网络的中间层是随机初始化的。
然后联合训练新网络。
还有一种选择，由\cite{Yu+al-2010}提出，将先前训练多层感知机的\emph{输出}，以及原始输入，作为每个附加阶段的输入。
...
\fi
\caption{一种形式的贪心监督预训练的示意图~\citep{Bengio-nips-2006-small}。
(a)我们从训练一个足够浅的架构开始。
(b)同一个架构的另一描绘。
...
另一个与监督预训练有关的方法扩展了迁移学习的想法：\cite{yosinski-nips2014}在一组任务上预训练了$8$层权重的深度卷积网络（1000个ImageNet对象类的子集），然而用该网络的前$k$层初始化同样规模的网络。
...
一个和跳跃连接相关的想法是添加和网络中间隐藏层相连的输出的额外副本，如GoogLeNet~\citep{Szegedy-et-al-arxiv2014}和深度监督网络\citep{Lee-et-al-2014}。
...
正如\secref{sec:poor_correspondence_between_local_and_global_structure}探讨的，许多优化挑战都来自于代价函数的全局结构，不能仅通过局部更新方向上更好的估计来解决。
...
延拓法也和参数中加入噪声的模拟退火紧密相关\citep{Kirkpatrick83}。
延拓法在最近几年非常成功。
参考~\cite{Mobahi+Fisher-AAAI2015}了解近期文献的概述，特别是在~AI~方面的应用。
...
\cite{Bengio+al-2009}指出被称为课程学习或者塑造, % ？ 整的方法可以被解释为延拓法。
...
\cite{Bengio+al-2009}验证这一策略为延拓法，通过增加简单样本的影响（通过分配它们较大的系数到代价函数，或者更频繁地采样），先前的$J^{(i)}$会变得更容易。
...
课程学习被证实为与人类教学方式一致\citep{Khan+Zhu+Mutlu-2011}：
教师刚开始会展示更容易、更典型的示例，然后帮助学习者在不太显然的情况下提炼决策面。
在人类教学上，基于课程学习的策略比基于样本均匀采样的策略\emph{更有效}，也能提高其他学习策略的效率\citep{Basu+Christensen-AAAI2013}。

% -- 319 --

课程学习研究的另一个重要贡献体现在训练循环神经网络捕获长期依赖：
\cite{Zaremba+Sutskever-arxiv2014}发现使用\emph{随机课程}获得了更好的结果，其中容易和困难的示例混合在一起，随机提供给学习者，更难示例（这些具有长期依赖）的平均比例在逐渐上升。
